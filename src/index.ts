...
export class Surepass {
  ...
  constructor(config: SurepassConfig) {
    if (!config.baseUrl.trim())
      throw new SurepassConfigError("baseUrl is required");
    if (!config.token.trim())
      throw new SurepassConfigError("token is required");
    this.#baseUrl = config.baseUrl.replace(/\/$/, "");
    this.#token = config.token;
    this.#fetch = config.fetch ?? globalThis.fetch;
    if (!this.#fetch)
      throw new SurepassConfigError("A fetch implementation is required");
  }
  ...
  async verifyGstin(
    ...
    const response = await this.#fetch(
      `${this.#baseUrl}/api/v1/corporate/gstin`,
      {
        method: "POST",
        headers: {
